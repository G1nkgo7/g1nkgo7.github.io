...
const isChineseSentence = (text) => {
  ...
  // Require CJK chars and reject Latin letters to keep output Chinese-only.
  return /[\u4e00-\u9fff]/.test(value) && !/[A-Za-z]/.test(value);
};

const fetchChineseHitokoto = async (api, retries = 6) => {
  for (let i = 0; i < retries; i += 1) {
    const response = await fetch(api);
    const data = await response.json();
    const quote = typeof data?.hitokoto === "string" ? data.hitokoto.trim() : "";

    if (isChineseSentence(quote)) {
      return data;
    }
  }
...
export default function initTyped(id) {
  ...
  if (hitokotoEnabled) {
    ...
    fetchChineseHitokoto(usrHitokotoAPI)
      .then((data) => {
        ...
        if (!data) {
          const fallbackEntries = normalizeSubtitleText(subtitleConfig.text);
          if (fallbackEntries.length !== 0) {
            createTyped(id, fallbackEntries, options);
          }
          return;
        }
